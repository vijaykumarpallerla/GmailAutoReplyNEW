...
def _safe_execute(func, retries=3, initial_delay=1):
    """Execute callable `func` with retries for network-related errors.

    `func` should be a callable with no arguments that performs the API call
    (for example: lambda: service.users().messages().list(...).execute()).
    """
    delay = initial_delay
    for attempt in range(retries):
        try:
            ...
            msg = str(e).lower()
            is_timeout = isinstance(e, (TimeoutError, socket.timeout)) or 'timed out' in msg or '10060' in msg or 'connection' in msg
            if attempt == retries - 1 or not is_timeout:
                raise
            time.sleep(delay)
            delay *= 2
...
def gmail_pull_for_user(user, q: str = 'newer_than:1h', max_results: int = 10) -> dict:
    ...
    try:
        ...
        if start_history_id:
            try:
                history_resp = _safe_execute(lambda: service.users().history().list(userId='me', startHistoryId=start_history_id, historyTypes=['messageAdded']).execute())
        ...
        if not start_history_id:
            ...
            msg_list = _safe_execute(lambda: service.users().messages().list(userId='me', q=query, maxResults=max_results).execute())
            messages = msg_list.get('messages', [])
            # Get profile to establish current historyId
            profile = _safe_execute(lambda: service.users().getProfile(userId='me').execute())
        ...
        for m in messages:
            ...
            try:
                msg = _safe_execute(lambda: service.users().messages().get(userId='me', id=m['id'], format='metadata').execute())
            except Exception as e:
                error_str = str(e)
            ...
            for rule in matched_rules:
                ...
                if cache_key not in _signature_cache:
                    try:
                        sendas_list = _safe_execute(lambda: service.users().settings().sendAs().list(userId='me').execute())
                        gmail_signature = ''
                ...
                for att in (action.attachments or []):
                    try:
                        ...
                        if content_base64:
                            ...
                            content = base64_module.b64decode(content_base64)
                            ...
                            print(f"[DEBUG] gmail_service attachment has no content in database: {name}", file=sys.stderr)
                    except Exception as _e:
                        print(f"[DEBUG] gmail_service attachment error for {att.get('name')}: {_e}", file=sys.stderr)
                ...
                try:
                    ...
                    sent_msg = _safe_execute(lambda: service.users().messages().send(userId='me', body={'raw': raw, 'threadId': thread_id}).execute())
                    sent_count += 1
        ...
        error_str = str(e)
        low = error_str.lower()
        # Authorization issues: tell caller so UI can prompt re-login
        if 'unauthorized' in low or 'invalid_grant' in low or 'invalid_grant' in error_str:
            ...
            return {'error': 'auth', 'message': error_str}
        # Network/timeout errors: return a network error so scheduler can continue
        if isinstance(e, (TimeoutError, socket.timeout)) or 'timed out' in low or '10060' in low or 'connection' in low:
            print(f"[WARN] Network error during Gmail pull for user {user.username}: {error_str}", file=sys.stderr)
            return {'error': 'network', 'message': error_str}
        # Other exceptions: re-raise to preserve original behavior
        raise
