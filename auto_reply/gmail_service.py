...
def _attachment_exists_fresh(path: str) -> bool:
    """
    Check if attachment exists in storage - NO CACHE, always check fresh.
    Always uses personal key from user's Cloudinary credentials only.
    Falls back silently if personal key not available (no error).
    """
    try:
        exists = default_storage.exists(path)
        print(f"[DEBUG] Checked attachment (fresh, no cache): {path} = {exists}", file=sys.stderr)
        return exists
    except Exception as e:
        print(f"[DEBUG] Error checking attachment existence (no cache): {path} - {e}", file=sys.stderr)
        return False
            ...
            for rule in matched_rules:
                ...
                for att in (action.attachments or []):
                    try:
                        ...
                        if path and _attachment_exists_fresh(path):
